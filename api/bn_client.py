...
The client follows :class:`BaseExchangeClient` so existing strategies can switch
to Binance Futures without code changes. Endpoints match Binance Futures USDT-M
REST API semantics.
"""
...
class BinanceClient(BaseExchangeClient):
    """REST client for Binance Futures USDT-margined API."""
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.use_testnet = bool(
            cfg.pop("use_testnet", False)
            or cfg.pop("testnet", False)
            or cfg.get("network") == "testnet"
        )

        self.base_url = cfg.get("base_url") or (
            self.TESTNET_URL if self.use_testnet else self.MAINNET_URL
        )
        self.recv_window = int(cfg.get("recv_window", 5000))
        ...
        self.logger = setup_logger("api.binance_client")

        if self.use_testnet:
            self.logger.info("BinanceClient initialised on testnet (%s)", self.base_url)
        else:
            self.logger.info("BinanceClient initialised on mainnet (%s)", self.base_url)

    # ------------------------------------------------------------------
    # BaseExchangeClient abstract methods
    # ------------------------------------------------------------------
    ...
    def make_request(
        ...
        """Perform HTTP request and return JSON dict or {'error': ...}."""
        ...
        if signed:
            if not secret_key:
                return {"error": "Missing secret key for signature"}
        ...
        for attempt in range(int(retry_total)):
            try:
                ...
                if response.status_code == 429:
                    ...
                    self.logger.warning("Binance API rate limit hit, retrying in %.1f s", wait_time)
                ...
                if attempt < retry_total - 1:
                    self.logger.warning("Binance API request error (%s), retrying...", exc)
                    time.sleep(1)
                    continue
    ...
    def _ensure_symbol_cache(self) -> None:
        ...
        if isinstance(info, dict) and info.get("error"):
            self.logger.error("Failed to fetch exchange symbols: %s", info["error"])
            self._symbol_cache = {}
    ...
    def _resolve_symbol(self, symbol: Optional[str]) -> Optional[str]:
        ...
        return self._symbol_cache.get(self._lookup_key(sanitized), sanitized)
    ...
    def _unknown_symbol_error(self, symbol: str) -> Dict[str, Any]:
        ...
        message = f"Unable to resolve symbol: {symbol}"
        if suggestions:
    ...
    # ------------------------------------------------------------------
    # Public REST wrappers used by strategies
    # ------------------------------------------------------------------

    def get_deposit_address(self, blockchain: str) -> Dict[str, Any]:
        return {"error": "Binance Futures does not expose deposit address via this API"}
    ...
    def get_collateral(self, subaccount_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        if isinstance(result, dict) and result.get("error") and "assets" not in result:
            return result
        assets_payload = []
    ...
    def execute_order(self, order_details: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not symbol:
            return {"error": "Missing trading pair", "status_code": 400}
        ...
        if not order_type:
            return {"error": "Missing order type"}
    ...
    def get_market_limits(self, symbol: str) -> Optional[Dict[str, Any]]:
        ...
        if not symbol_info:
            self.logger.error("Symbol %s missing in exchange metadata", resolved_symbol)
            return None
    ...
    # ------------------------------------------------------------------
    # Public helpers for strategy compatibility
    # ------------------------------------------------------------------

    def resolve_symbol(self, symbol: Optional[str]) -> Optional[str]:
        return self._resolve_symbol(symbol)

    def normalize_order_fields(self, order: Dict[str, Any]) -> Dict[str, Any]:
        return self._normalize_order_fields(order)
